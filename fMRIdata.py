...
def construct_data():
    ...
    # fSMCI, fSlabel = load_fmri_data(mcitype='SMC/')
    EMCI_DTI = load_dti_graph(mcitype='EMCI/')  # label same as above
    # SMCI_DTI = load_dti_graph(mcitype='SMC/')
    ...
    # fmri = np.vstack((fNC, fSMCI)) #NC SMC
    # dti = np.vstack((NC_DTI, SMCI_DTI))
    # fmri = np.vstack((fNC, fLMCI)) # SMCI EMCI
    # dti = np.vstack((NC_DTI, LMCI_DTI))
    labels = np.vstack((fNlabel, fElabel))  # share labels # print(labels.shape)
    return fmri, dti, labels
...
# SL
fMRI, DTI, Labels = construct_data()
NE_fMRI = fMRI[permutation, :]
NE_Labels = Labels[permutation, :]
NE_DTI = DTI[permutation, :]

np.save("need/NE_dti_draph.npy", NE_DTI)
np.save('need/NE_label.npy', NE_Labels)
# print(LN_fMRI)
NE_fMRI_FCN = bulid_fcn(NE_fMRI)
# print(NS_fMRI_FCN)
print("Fcn network Construction successful ! !")

NE_fMRI_fea = extra_fea(NE_fMRI_FCN)
np.savetxt('need/NE_fuse_fea.txt', NE_fMRI_fea)

unimage = np.array(bulid_unimage())
adj = bulid_graph(NE_fMRI_fea, unimage)
print("Unimage Construction successful ! !")
np.savetxt('dataset/NE_fuse_graph.txt', adj)
